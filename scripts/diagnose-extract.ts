...
import { callEngine, SCHEMAS, isEntityType, DEFAULT_GEMINI_MODEL, type Engine, type EntityType, type AttemptLog } from '../lib/extractEngines'
...
function makeFile(buffer: Buffer, mimeType: string): File {
  return new File([new Uint8Array(buffer)], 'diagnose-input', { type: mimeType })
}
...
async function runOnce(engine: Engine, file: File | null, text: string | null, entityType: EntityType, attemptNum: number, geminiModel: string, locale?: string) {
  ...
  try {
    ...
    const totalMs = Date.now() - started
    console.log(`  RESULT: FAILED after ${totalMs}ms — ${err instanceof Error ? err.message : String(err)}`)
...
async function main() {
  ...
  if (!isEntityType(entityTypeArg)) {
    console.error(`Invalid --type "${entityTypeArg}". Must be one of CARD, VOUCHER, REFUND, WARRANTY.`)
    process.exit(1)
  }
  ...
  if (args.image) {
    ...
    console.log(`Input: image file "${args.image}" (${buffer.length} bytes, ${guessMimeType(args.image)})`)
  } else if (args['text-file']) {
    text = fs.readFileSync(args['text-file'], 'utf8')
    console.log(`Input: text file "${args['text-file']}" (${text.length} chars)`)
  } else if (args.text) {
    text = args.text
    ...
    console.log(`Input: fixture image "${DEFAULT_IMAGE_FILE}" (${buffer.length} bytes) — edit this file in place to change it`)
  } else {
  ...
  for (const engine of engines) {
    ...
    const avg = Math.round(s.totalMs.reduce((a, b) => a + b, 0) / s.totalMs.length)
    console.log(`${engine}: ${s.ok}/${s.ok + s.fail} succeeded, avg ${avg}ms (${s.totalMs.join(', ')}ms per run)`)
  }
}
