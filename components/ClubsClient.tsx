      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label={t.clubName} required>
          <input
            name="name"
            required
            placeholder="e.g. Shufersal Club"
        ...
        <Field label={t.idTypeLabel} required>
          <select name="idType" required defaultValue="" className={inputClass}>
            ...
            {(
              Object.entries(t.idTypes) as [keyof typeof t.idTypes, string][]
            ).map(([value, label]) => (
...
function ClubDetailModal({
  ...
  const addedByName =
    showAddedBy && club.createdBy && attributionNames[club.createdBy]
        ...
        {club.expiresAt && (
          ...
          >
            ...
            >
              ...
              {soon(club.expiresAt) && (
                <ExpiryDaysBadge expiresAt={club.expiresAt!} />
              )}
            </p>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label={t.expirationOptional}>
          <input
            name="expiresAt"
            type="date"
            defaultValue={club.expiresAt ? club.expiresAt.slice(0, 10) : ""}
            className={inputClass}
          />
        </Field>
        <Field label={t.notesOptional}>
        ...
        <div className="flex items-center gap-2 mt-0.5">
          {club.ownerName && (
            <span className="text-xs text-slate-400 truncate">
              <HighlightMatch text={club.ownerName} query={query} />
            </span>
          )}
          {club.idType && (
            <span className="text-xs text-slate-400">
              {t.idTypes[club.idType]}
            </span>
          )}
        </div>
      </div>
  ...
  return (
    <div className="clubs-page space-y-6" dir={dir}>
      <div className="clubs-page-header flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-800 dark:text-neutral-400">
          {t.clubsTab}
        </h1>
        <button
          type="button"
