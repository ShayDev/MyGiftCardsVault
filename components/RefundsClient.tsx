...
function RefundRow({
  ...
  return (
    <div
      dir={dir}
      className={`refund-row w-full rounded-2xl border shadow-sm hover:shadow-md transition-all p-4 flex items-center gap-3 pr-2 overflow-hidden ${
        expiringSoon
    ...
    >
      ...
      <button
        type="button"
        onClick={onClick}
        className="flex-1 min-w-0 flex flex-wrap items-center gap-x-3 gap-y-1 text-start"
      >
        <span
        ...
        <span
          className={`flex-shrink-0 max-w-[45%] sm:max-w-none truncate px-2.5 py-1 rounded-full text-xs font-semibold ${providerColor(refund.provider)}`}
        >
          <HighlightMatch text={refund.provider} query={query} />
        </span>
        <div className="flex-1 min-w-[64px]">
          <div className="flex items-baseline gap-2" dir="ltr">
            <p className="text-sm font-mono font-semibold text-slate-800 dark:text-neutral-100 truncate">
              {formatAmount(
        ...
        {refund.expiresAt && (
          // basis-full drops this to its own line on narrow screens instead of
          // squeezing the amount/reference into unreadable slivers — see the
